...
- Always write the spec content to user-visible Markdown files in the repository; do not provide the spec only in chat.
...
- All specs MUST be written as `.md` files so the user can open, review, and edit them directly.
- If feature_slug is missing, derive a short kebab-case slug from the request and use it consistently in the `.md` file paths.
...
- Create or update the Markdown spec files before reporting completion. The final response is only a summary with links/paths, not the canonical spec.
